        ...
        $('#btnModif').on('click', function(e){
            ...
            $('#btnRegisterConv').on('click',function(e){
                ...
                $('#btnModifConv').on('click',function(e){
                    ...
                    // Affichage de la modal pour la saisie des documents relatifs à un produits
                    $('body').on('click', '.saisieDocProduit', function(){

                        $('#btnModifDoc').hide();
                        $('#btnRegisterDoc').show();
                        $('#table').hide();
                        $('#formDocProduits').show();
                        $('#loaddocument').hide();
                        //Récupération des détails de la ligne en cours de modification
                        produit                                        = $(this).data('id');

                        //Rédefinition du titre de la modal
                        $('#docProduitsTitle').empty();
                        $title                                       = $('#docProduitsTitle').text(`Saisie des documents relatifs au produit [ ${produit.r_nom_produit} ]`);
                        //Récupération des documents
                        flisteDocsProduit(produit.id);

                        $('#docProduits').modal('show');

                    });

                    //Cette fonction retourne la liste des produits

                    var flisteDocsProduit = (productId)=>{
                        $.ajax({
                            url: "{{ url('/documents') }}/"+productId,
                            type:"GET",
                            contentType: false,
                            success:function(response){
                                $('#documentTable').empty();
                                response._result.forEach((item, index)       => {
                                $("#documentTable").append(`
                                    <tr>
                                        <td class="fw-semibold">${item.r_libelle}</td>
                                        <td class="fw-semibold">${item.created_at}</td>
                                        <td class="fw-semibold">
                                            <i class="far fa-pen-to-square editDocs fontawesome text-primary m-1" data-id='${JSON.stringify(item)}' title="Modifier le document [ ${item.r_libelle} ]"></i>
                                            <i class="fa-solid fa-file-pdf affichedoc fontawesome text-warning m-1" data-id='${JSON.stringify(item)}' title="Voir le document [ ${item.r_libelle} ]"></i>
                                            <i class="fa-solid fa-trash-can fontawesome text-danger m-1" data-id='${JSON.stringify(item.id)}' title="Suprimer le document [ ${item.r_libelle} ]"></i>
                                        </td>
                    ...
                    //Click du boutton voir les documents saisies
                    $('#voirdocs').on('click', function(){
                        $('#formDocProduits').hide(1000);
                        $('#table').show(1000);
                    });

                    //Click du boutton saisies documents
                    $('#saisiedocs').on('click', function(){
                        $('#formDocProduits')[0].reset();
                        $('#btnRegisterDoc').show();
                        $('#btnModifDoc').hide();
                        $('#loaddocument').hide();

                        $('#table').hide(1000);
                        $('#formDocProduits').show(1000);
                    });


                    //Affichage des documents
                    $('body').on('click','.affichedoc', function(){
                        datas = $(this).data('id');
                        window.open(datas.document_path, '_blank');
                    });

                    //Consulatation des détails des documents
                    $('body').on('click','.editDocs', function(){
                        ...
                        $('#r_description').val(datas.r_description);
                        $('#loaddocument').attr('href',datas.document_path);
                    ...
                    //Enregrement de documents
                    $('#btnRegisterDoc').on('click',function(e){
                        e.preventDefault();
                        var ext = '';
                        var formData                                     = new FormData($("#formDocProduits")[0]);
                        //formData.append('_method', 'post');
                        formData.append('_token', "{{ csrf_token() }}");
                        formData.append('r_produit', produit.id);
                        ext = formData.get('document').name.split('.')[1];
                        ...
                        /*********************************************** Control des champs *********************************************/
                        if ( obj.r_libelle                              == '' || obj.r_libelle == undefined) {
                            $('#erreur_libelle').empty();
                            $('#erreur_libelle').append('Veuillez saisir le libellé du document');
                            return;
                        }else{
                            $('#erreur_libelle').empty();
                        }

                        if ( obj.document                              == '' || obj.r_libelle == undefined) {
                            $('#erreur_document').empty();
                            $('#erreur_document').append('Veuillez charger le document');
                            return;
                        }else{
                            $('#erreur_document').empty();
                        }
                        ...
                        if ( ext                              !== 'pdf' || ext == null) {
                            $('#erreur_document').empty();
                            $('#erreur_document').append('Veuillez charger un document valide');
                            return;
                        }else{
                            $('#erreur_document').empty();
                        }
                        ...
                        $('#btnModifDoc').on('click',function(e){
                        e.preventDefault();
                        var ext = '';
                        var formDataUpdate                                     = new FormData($("#formDocProduits")[0]);
                        //formData.append('_method', 'post');
                        formDataUpdate.append('_token', "{{ csrf_token() }}");
                        ext = formDataUpdate.get('document').name.split('.')[1];
                    ...
                    function readURL(input) {
                        if (input.files[0].name !== 'pdf') {
                            $('erreur_document').empty();
                            $('erreur_document').append('Format du document incorrect');
                            $(this).attr('value','');
                            return;
                        }
                    }
                ...
                <div class="content">
                    ...
                    <div class="row">
                        <div class="col">
                            <button type="button" class="btn btn-primary mb-2" id="newProduct"
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <form method="post" id="formProduits" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        <label for="r_type_client" class="form-label is-required">Type de client</label>
                                    ...
                                    <div class="mb-3">
                                        <label for="r_nom_produit" class="form-label is-required">Nom du produit</label>
                ...
                <!-- Modal Saisie des documents relatifs à un produit-->
                <div class="modal fade" id="docProduits" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="docProduits" aria-hidden="true">
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="docProduitsTitle"></h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"><span class="visually-hidden">Close</span></button>
                            </div>
                            <hr>
                            <div class="modal-body">
                                ...
                                <div class="row mb-1">
                                    <div class="col">
                                        <button class="btn btn-primary float-end" id="voirdocs">
                                            <i class="fa-solid fa-file-pdf"></i> &nbsp; Voir les documents
                                          </button>
                                          <button class="btn btn-info float-end" id="saisiedocs">
                                ...
                                <!-- Formulaire de saisie des produits -->
                                <form method="post" id="formDocProduits" enctype="multipart/form-data">
                                    @csrf


                                    <div class="mb-3">
                                        <label for="r_libelle" class="form-label">Libellé du document</label>
                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <div class="mb-3">
                                                <label for="document" class="form-label">Charger le document <span style="color: red;" >(pdf)</span> </label>
                                                <input type="file" class="form-control" name="document" id="document" onchange="readURL(this);">
                                                <span class="error" id="erreur_document" ></span>

                                            </div>
                                        </div>
                                    </div>

                                    <a href="" target="_blank" id="loaddocument" class="text-primary">Voir le document</a>

                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" title="Cliquer pour fermer">Fermer</button>
                                        <button class="btn btn-primary" id="btnRegisterDoc" title="Cliquer pour enregistrer" >
                                            Enregistrer &nbsp;
                                            <div class="spinner-border spinner-border-sm spinnerRegister" role="status" hidden>
                                                <span class="visually-hidden">Loading...</span>
                                            </div>
                                        </button>
                                        <button class="btn btn-primary" id="btnModifDoc" title="Cliquer pour enregistrer la modification">
                                            Modifier &nbsp;
                                            <div class="spinner-border spinner-border-sm spinnerModif" role="status" hidden>
                                                <span class="visually-hidden">Loading...</span>
                                            </div>
                                ...
                                <table class="table table-bordered table-striped table-vcenter fs-sm" id="table">
                ...
                <!---------------------- Fin Modal pour la saisie des documents relatifs à un produit---------------------------------->
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-muted">
                                        ...
                                        <i class="fa-solid fa-file-pdf saisieDocProduit fontawesome text-info m-1" data-id="{{ $produit}}" title="Enregistrer les documents du produit [ {{ $produit->r_nom_produit }} ]"></i>
